...
{
    int len=par.length;
    auto op=HamTb1(len,false);//CadenitaAA(par.length,par.periodic,par.mu).toMPO();//
    op.Sweep(); op.PrintSizes("Hamtb=");
...
void TestDMRGCV(const Parameters& par,int id, int n_id)
{
    ...
    auto op=HamTb1(len,false);//CadenitaAA(par.length,par.periodic,par.mu).toMPO(); op.Sweep(); op.PrintSizes("Ham=");
    op.decomposer=MatQRDecomp;
